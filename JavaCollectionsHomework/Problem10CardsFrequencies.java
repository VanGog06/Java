...
should be listed only once in the output. Examples:
...
public class Problem10CardsFrequencies {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < cards.length; i++) {
            cards[i] = cards[i].replaceAll("[♥♣♦♠]", "");
        }

        LinkedHashMap<String, Double> playingCards = new LinkedHashMap<>();

        for (String card : cards) {
            Double count = playingCards.get(card);
            if (count == null) {
                count = 0d;
            }

            playingCards.put(card, count + 1);
        }

        for (Map.Entry<String, Double> pair : playingCards.entrySet()) {
            System.out.printf("%s -> %.2f%%", pair.getKey(), (pair.getValue() / cards.length) * 100);
            System.out.println();
        }
    }
}
